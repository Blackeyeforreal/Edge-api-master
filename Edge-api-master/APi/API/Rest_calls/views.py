...
class GamePlay(APIView):
    ...
    def match(request):
        ...
        for i in range(2):
            Eleven_players = []
            team_name = request.data["challenger_team_name"] if i == 0 else request.data["challenged_team_name"] 
            bowlers = []
            user_id = CricketTeam.objects.get(id = challenger[i]).userId
            #print(Challenges.objects.get(challenge_id=  request.data["challenge_id"]), user_id)
            Eleven_players_id = PlayerEleven.objects.filter( challenge_id__challenge_id =   request.data["challenge_id"] ,  user_id__id=  user_id.id)
            ...
            no_of_wickets = random.randint(1,8)
            wicket_over_list= sorted(random.sample(range(1, 21), no_of_wickets))
            
            def generate_random_dictionary(numbers):
                random_dict = {}
                for num in numbers:
                    random_dict[num] = random.randint(1, 6)
                return random_dict

            print(len(Eleven_players),"<---players")
            #bowling types = fast / medium /off spin / leg spin

            random_dict = generate_random_dictionary(wicket_over_list)

            print("random dick ---->> ", random_dict)
            for turn in range(1,121):
                over_no = turn//6
                ball_no = 6 if turn%6 ==0 else turn%6
                bowl_type = ""
                ...
                #striker_rating= Eleven_players[current_striker_index][bowl_type]
                print(current_striker_index,current_nonStriker_index)
                if (current_striker_index >10) or (current_nonStriker_index > 10):
                    print("--> csi",current_striker_index,current_nonStriker_index )
                    break
                ...
                if(over_no in random_dict.keys() and ball_no == random_dict.get(over_no) ):
                    outcome=-1
                else:
                    outcome = 9
